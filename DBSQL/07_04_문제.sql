...
SELECT
	...
	;
-- 1. 수강학생별로 수강중인 과목의 가장바른 시작일자와 가장 늦은 끝일자를 출력
-- view를 사용하여 학생별 수강일자 최저시작일, 최대종요일
SELECT name
		, min(start_dt) as sd
		, max(end_dt) as ed
FROM LEARN_VIEW
...
-- sub query를 사용하여 학생별 수강일자 최저시작일, 최대종료일
select t1.name
	, min(t1.start_dt)
	, max(t1.end_dt)
from (
...
-- select join으로 연결한 후에 각 테이블의 컬럼을 group, 집합연산을 사용하여 학생별 수강일자 최저시작일, 최대종요일을 보여라
SELECT ct.name
		, min(lt2.start_dt)
		, max(lt2.end_dt)
FROM learn_tbl lt
...
-- 3. 각과목별로 최우수점수 학생 이름과 score가 맨위로 정렬
-- 최고점수 순서로 정렬을 해본다.
...
-- 3. 과목정보에 과목명, 가장 높은 점수 1개, 가장 높은 점수의 학생이름 출력 해결
-- select a, b, (select a from t .. 1개의 행과 1개의 컬럼 값만 가져와야 한다.), c, d, from (데이터)
SELECT lt.subject, lt.id
	,(SELECT ln.score
		FROM learn_tbl ln
		WHERE ln.lecture_id = lt.id -- 수강정보테이블의 과목이 가은 행들
...
-- 4. 과목정보에 과목명, 가장 낮은 점수 1개, 가장 낮은 점수의 학생이름 출력
...
FROM SALE_VIEW
group by name order by price desc;
